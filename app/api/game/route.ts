...
import { loadWebGame } from "@/src/web/server/gameApp";
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json(await loadWebGame(session.gameId, session.playerId), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
